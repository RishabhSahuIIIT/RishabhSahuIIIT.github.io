...
\begin{comment}
description:
A hands-on project to learn Rust, built by working through the capstone exercise of the official Rust book (``The Rust Programming Language''). Implements a multithreaded HTTP server backed by a thread pool, used as a vehicle to get practical experience with Rust's ownership model and concurrency primitives -- Arc, Mutex, and channels.
\end{comment}
...
papers end-to-end: parsing with PyMuPDF, structured (map-reduce) summarisation,
cross-paper synthesis, and mini-survey generation with inline citations. A dual
Ollama architecture runs orchestration (a lightweight Llama 3.2 for routing) and
task execution (Llama 3.1) on separate local instances to prevent blocking, with
every decision and output logged to timestamped JSONL trace files for full
observability.
\end{comment}
\subsection*{Agentic AI Research Assistant \hfill {\normalfont\itshape\small Artificial Intelligence}}

\noindent\hspace{15pt}{\fontsize{11.5pt}{13.5pt}\selectfont \textbf{Keywords:} Python, Moya framework, Ollama, Llama 3.1/3.2, PyMuPDF, Multi-agent systems \hfill \href{https://github.com/RishabhSahuIIIT/AgenticAIResearchAssistant_Moya}{\faIcon{link}\textbf{Project repository}}}
...
computed entirely on the client. The app is deployed live at citestat.vercel.app.
A 3-person team project.
\end{comment}
...
presence. A 3-person team project.
\end{comment}
...
\begin{comment}
description:
Two independent distributed systems built over gRPC and Protocol Buffers: a
master--worker k-Nearest-Neighbours engine that partitions a dataset across
worker servers and merges per-worker top-k results, and a fault-tolerant
task-queue manager that pushes CPU/IO jobs to capability-matched workers via
server-side streaming and re-queues work abandoned by failed workers.
\end{comment}
\subsection*{Distributed k-NN \& Task Queue over gRPC \hfill {\normalfont\itshape\small Distributed Systems}}

\noindent\hspace{15pt}{\fontsize{11.5pt}{13.5pt}\selectfont \textbf{Keywords:} Python, gRPC, Protocol Buffers, Distributed Systems, Concurrency \hfill \href{https://gitlab.com/iiith2426-ris/assignments/ds/knn_tq_grpc}{\faIcon{link}\textbf{Project repository}}}
...
\begin{comment}
description:
A distributed parallel implementation of Boruvka's minimum-spanning-tree
algorithm in MPI, orchestrated on a SLURM cluster, with a performance harness
that measures speedup, parallel efficiency, and computation-versus-communication
profiling across process counts.
\end{comment}
\subsection*{Parallel Minimum Spanning Tree (MPI + SLURM) \hfill {\normalfont\itshape\small Parallel Computing}}
...
\begin{comment}
description:
A collection of out-of-tree LLVM optimization and analysis passes built on the
new pass manager and loaded into opt as plugins -- CFG printing, static
function-call counting, constant propagation, identity-operand instruction
rewriting, and dead-code elimination driven by live-variable dataflow analysis.
A 2-person team project.
\end{comment}
...
\begin{comment}
description:
A compiler frontend that translates a subset of the Racket language to LLVM IR
through a full lexer--parser--semantic-analyzer--codegen pipeline, built
incrementally to add booleans, while loops, fixed-length tuples, and recursive
user-defined functions. A 2-person team project.
\end{comment}
\subsection*{Racket-to-LLVM Compiler Frontend \hfill {\normalfont\itshape\small Compilers}}

\noindent\hspace{15pt}{\fontsize{11.5pt}{13.5pt}\selectfont \textbf{Keywords:} C++, LLVM, Compiler Design, Recursive-Descent Parsing, Code Generation \hfill \href{https://gitlab.com/iiith2426-ris/assignments/compilers/rlcp}{\faIcon{link}\textbf{Project repository}}}
...
\begin{comment}
description:
Trajectory generation for a non-holonomic robot: fitting order-5 Bernstein
polynomials to paths under velocity, acceleration, and orientation boundary
constraints, then using the Cross-Entropy Method to search for waypoints that
steer the trajectory around static obstacles.
\end{comment}
\subsection*{Bernstein Trajectory Fitting \& CEM Obstacle Avoidance \hfill {\normalfont\itshape\small Robotics}}

\noindent\hspace{15pt}{\fontsize{11.5pt}{13.5pt}\selectfont \textbf{Keywords:} Python, NumPy, Trajectory Optimization, Cross-Entropy Method, Robotics \hfill \href{https://gitlab.com/iiith2426-ris/assignments/robotics-planning-and-navigation/bernstein_cem}{\faIcon{link}\textbf{Project repository}}}
...
\begin{comment}
description:
Sampling-based motion planning for a 7-DOF Franka Panda arm in a cluttered
scene, using bidirectional RRT (RRT-Connect) with custom joint-limit-aware
sampling and extension, simulated and recorded in PyBullet.
\end{comment}
\subsection*{Franka Robot Arm Motion Planning \hfill {\normalfont\itshape\small Robotics}}

\noindent\hspace{15pt}{\fontsize{11.5pt}{13.5pt}\selectfont \textbf{Keywords:} Python, PyBullet, Motion Planning, RRT-Connect, Sampling-based Planning \hfill \href{https://gitlab.com/iiith2426-ris/assignments/robotics-planning-and-navigation/franka-robot-arm-navigation}{\faIcon{link}\textbf{Project repository}}}
...
\begin{comment}
description:
Five tabular reinforcement-learning methods implemented from scratch in NumPy --
Value Iteration, Policy Iteration, Monte Carlo control, SARSA(lambda), and
Q-learning -- solving a stochastic Windy Gridworld and comparing model-based
planning against model-free learning via convergence and regret analysis.
A 5-person team project.
\end{comment}
\subsection*{Tabular Reinforcement Learning -- Windy Gridworld \hfill {\normalfont\itshape\small Reinforcement Learning}}
