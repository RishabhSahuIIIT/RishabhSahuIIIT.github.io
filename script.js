...
(function () {
  ...
  /* Extract project descriptions from the RAW LaTeX, before any
     ...
     project. */
  function extractProjectDescriptions(rawText) {
    const map = {};
    ...
    while ((m = re.exec(rawText)) !== null) {
      ...
      const scanStart = Math.max(prevSub, prevSec, 0);
      const scanText  = rawText.slice(scanStart, m.index);

      let description = null;

      // Format 1: multi-line block — \begin{comment}...\end{comment}
      //           or \iffalse...\fi
      const blockRe = /(?:\\begin\{comment\}|\\iffalse\b)([\s\S]*?)(?:\\end\{comment\}|\\fi\b)/g;
      let bm;
      while ((bm = blockRe.exec(scanText)) !== null) {
        const body = bm[1];
        const dm = /^\s*description\s*:\s*([\s\S]*)$/i.exec(body);
        if (dm) {
          description = dm[1].replace(/\s+/g, ' ').trim();
          break;
        }
      }

      // Format 2: single-line  % description: <text>
      if (!description) {
        const lineRe = /(?:^|\n)[ \t]*%[ \t]*description[ \t]*:[ \t]*(.*?)(?=\r?\n|$)/i;
        const lm = lineRe.exec(scanText);
        if (lm && lm[1].trim()) description = lm[1].trim();
      }

      if (description) map[title] = description;
    }
    return map;
  }
  ...
  function parseResume(latex) {
    // Extract `% description:` comments BEFORE stripComments wipes them.
    const descriptions = extractProjectDescriptions(latex);
    ...
    projects.forEach(p => {
      if (descriptions[p.title]) p.description = descriptions[p.title];
    });
  ...
  function renderContact(c) {
    ...
    set('contact-phone',    c.phone, c.phone ? 'tel:' + c.phone.replace(/[^+\d]/g, '') : null);
    set('contact-linkedin', c.linkedin ? c.linkedin.replace(/^https?:\/\/(www\.)?linkedin\.com\/in\//, '') : '', c.linkedin);
    set('contact-github',   c.github   ? c.github.replace(/^https?:\/\/(www\.)?github\.com\//, '') : '', c.github);
    set('contact-gitlab',   c.gitlab   ? c.gitlab.replace(/^https?:\/\/(www\.)?gitlab\.com\//, '') : '', c.gitlab);
  }
  ...
  function renderEducation(entries) {
    ...
    el.innerHTML = entries.map(e => {
      ...
      return '<div class="timeline-row">' +
        ...
        '<div class="what">' + escapeHTML(e.institution || '') +
  ...
  function renderInternships(internships) {
    ...
    el.innerHTML = internships.map((intern, idx) => {
      ...
      return '<article class="work-item">' +
        ...
        '<div>' +
          '<h3>' + escapeHTML(intern.title || 'Internship') + '</h3>' +
  ...
  function renderProjects(projects, opts) {
          ...
          featuredEl.innerHTML = groups.map(g => {
            ...
            return '<div class="project-group work-group" id="work-cat-' + slug(g.category) + '">' +
              '<h3 class="project-group-head">' + escapeHTML(g.category) +
                ' <span class="project-group-count">' + g.projects.length + '</span></h3>' +
    ...
    /* ---- All projects — hide section if empty ---- */
    ...
    if (groupByCat && hasCategories) {
      ...
      allEl.innerHTML = groups.map(g =>
        '<div class="project-group" id="proj-cat-' + slug(g.category) + '">' +
          '<h3 class="project-group-head">' + escapeHTML(g.category) +
            ' <span class="project-group-count">' + g.projects.length + '</span></h3>' +
          '<div class="project-group-list">' +
            g.projects.map(p => renderCompactItem(p, compactOpts)).join('') +
          '</div>' +
        '</div>'
      ).join('');
    } else {
      allEl.innerHTML = rest.map(p => renderCompactItem(p, compactOpts)).join('');
    }
  }
  ...
  function showLoadError(err) {
    const attemptedUrl = (function () {
      try { return new URL('resume.tex', window.location.href).href; }
      catch (e) { return 'resume.tex (relative to ' + window.location.href + ')'; }
    })();
    ...
    if (state.projectsAllHasContent) {
      ...
      if (cats.length) {
        items.push({
          ...
          children: cats.map(c => ({ label: c, href: '#proj-cat-' + slug(c) }))
        });
      } else {
  ...
  function boot() {
    ...
    console.log('[Portfolio] Fetching resume.tex from', new URL('resume.tex', window.location.href).href);
    fetch('resume.tex')
      .then(r => {
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { parseResume, stripComments, stripFormatting, buildEntryList, buildAccomplishmentsList, applyContactConfig, groupByCategory };
